...
// Exactly two endpoints (see .env): one CometBFT RPC + one Cosmos REST (LCD).
...
export class ChainClient {
  ...
  private async request(
    ...
    throw new Error(`${RETRY_ATTEMPTS} attempts failed for ${base + path}: ${lastError}`);
  }
  ...
  private async rpcGet<T>(path: string): Promise<T> {
    ...
    if (body.error) {
      throw new Error(`rpc error for ${path}: ${body.error.message} ${body.error.data ?? ''}`);
    }
  ...
  lcdGet<T>(path: string, opts?: { height?: number }): Promise<T> {
    const headers =
      opts?.height !== undefined ? { 'x-cosmos-block-height': String(opts.height) } : undefined;
