...
import type { MonthlyBucket } from './statsSeries';
...
function statsDoc(overrides: Partial<ValidatorStatsMonthDoc> = {}): ValidatorStatsMonthDoc {
...
test('buildValidatorRow: average stake, latest cumulative withdraw, sold%, commission', () => {
  const sinkSales: SinkSaleDoc[] = [{ sink_address: 'binance', cumulative_sold: '3000000', timestamp: 2000 }];
...
test('buildValidatorRow clamps percentage_sold to 100 even if sold overshoots (e.g. price appreciation edge case)', () => {
  const sinkSales: SinkSaleDoc[] = [{ sink_address: 'binance', cumulative_sold: '99000000', timestamp: 2000 }];
  const row = buildValidatorRow(validator, [statsDoc()], sinkSales, 6);
...
test('buildNetworkMonthlyBuckets sums per-validator buckets day by day and derives calendar timestamps', () => {
  ...
  const [network] = buildNetworkMonthlyBuckets([validatorABuckets, validatorBBuckets], priceTimeline);
  ...
  assert.equal(network!.data.timestamp[0], Math.floor(Date.UTC(2026, 0, 1) / 1000));
