...
import { flattenPopulatedDays, type MonthlyBucket, type ValidatorStatsMonthDoc } from './statsSeries';
import { latestCumulativeByPair, type SinkSaleDoc } from './sinkSales';
...
export function buildValidatorRow(
  ...
  const average_total_stake =
    stakeSamples.length > 0 ? stakeSamples.reduce((a, b) => a + b, 0) / stakeSamples.length : 0;
  ...
  const total_withdraw =
    uatomToAtom(latest.total_withdrawn_reward, decimals) + uatomToAtom(latest.total_withdrawn_commission, decimals);
  ...
  const commission = validator.commission_rate ? Number(validator.commission_rate) * 100 : 0;
...
export function rankByPercentageSold(rows: ReadonlyArray<ValidatorRow>): Map<string, number> {
  const ranks = new Map<string, number>();
  for (const row of rows) {
    const rank = 1 + rows.filter((r) => r.percentage_sold > row.percentage_sold).length;
    ranks.set(row.operator_address, rank);
  }
  return ranks;
}

export function buildSummaryData(rows: ReadonlyArray<ValidatorRow>): SummaryData {
  ...
  const percentage_sold = total_withdraw_sum > 0 ? clampPercent((total_sold / total_withdraw_sum) * 100) : 0;
...
export function buildMetrics(summaryData: SummaryData, averagePrice: number): Metric[] {
  return [
    ...
    { id: 'price', color: '#31ADE6', title: 'Average ATOM Price', valueNative: averagePrice },
  ];
}
...
export function buildNetworkMonthlyBuckets(
  ...
  for (const buckets of byMonth.values()) {
    ...
    result.push({ year, month, data: { timestamp, total_stake, total_sold, price } });
  }
