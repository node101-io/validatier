...
test('first write in a month creates one doc with a 31-length null array, populated only at day-1 index', async () => {
  ...
  assert.equal(doc.self_stake.length, 31);
  assert.equal(doc.self_stake[4], '111'); // day 5 -> index 4
  ...
  assert.equal(doc.self_stake[0], null);
  assert.equal(doc.self_stake[30], null);
});
...
test('a second day in the same month reuses the same document (no path conflict) and does not disturb the first day', async () => {
  ...
  const doc = docs[0];
  assert.equal(doc.self_stake[4], '111'); // day 5 from the previous test untouched
  assert.equal(doc.self_stake[5], '333'); // day 6 -> index 5
...
test('a different month for the same validator creates a separate document', async () => {
  ...
  assert.equal(docs[1].self_stake[0], '555');
});
