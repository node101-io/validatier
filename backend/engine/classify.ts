...
// `ctx` is threaded through only so markHolderStatus can stamp last_height/
// last_ts on every edge it flips — see that function's comment.
export function classifyRecipient(t: RealTransfer, ctx: BlockCtx): void {
  // IBC-out (docs/01 "Yöntem A"): same-msg_index ibc_transfer event. Terminal —
  // money left cosmoshub, we only track this chain.
  if (t.is_ibc_out) {
    markHolderStatus(t.recipient, 'realized', 'ibc_out', ctx.height, ctx.ts);
    return;
  }
  ...
  if (sink) {
    markHolderStatus(t.recipient, sink.tier === 1 ? 'realized' : 'suspected', sink.kind, ctx.height, ctx.ts);
    return;
  }

  // Tier 2 heuristic (single signal per lead dev: high in-degree only).
  if (inDegreeOf(t.recipient) >= config.tier2MinIndegree) {
    markHolderStatus(t.recipient, 'suspected', 'structural', ctx.height, ctx.ts);
    return;
  }
