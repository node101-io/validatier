...
with tabs[0]:
    ...
    if summary_df.empty:
        ...
        c1.metric("Total Withdrawn Rewards", f"{total_withdrawn:,.0f} ATOM")
        c2.metric("Total Sold (realized)", f"{total_sold:,.0f} ATOM")
        c3.metric("Average Sold %", f"{avg_sold_pct:.1f}%" if pd.notna(avg_sold_pct) else "n/a")

        if price:
            st.metric("Total Sold (USD)", f"${total_sold * price:,.0f}")
        ...
        st.dataframe(
            top_sellers[["moniker", "sold_pct", "sold_atom", "total_withdrawn_atom"]],
            width="stretch",
        )
        ...
        st.dataframe(
            bottom_sellers[["moniker", "sold_pct", "sold_atom", "total_withdrawn_atom"]],
            width="stretch",
        )
        ...
        st.dataframe(
            top_sold_amount[["moniker", "sold_atom", "sold_usd", "sold_pct"]],
            width="stretch",
        )
...
with tabs[1]:
    if not selected_operator_address:
        ...
        if row.empty:
            ...
            c2.metric("Commission", row["commission_rate"])
            ...
            if not sales_df.empty:
                ...
                fig2 = px.area(agg, x="date", y="cumulative_sold_atom", title="Cumulative sold — total across all sinks (ATOM)")
                fig2.update_traces(mode="lines+markers")
                st.plotly_chart(fig2, use_container_width=True)
            else:
                st.info("No validator_sink_sales data yet for this validator.")
...
with tabs[2]:
    ...
    st.dataframe(
        display_df[
            ...
            columns={
                "sold_pct_rank": "rank",
                "total_stake_atom": "total_stake (ATOM)",
                "total_withdrawn_atom": "total_withdrawn (ATOM)",
                "sold_atom": "sold (ATOM)",
                "sold_pct": "sold %",
            }
        ),
...
# ---------------------------------------------------------------- Sink Sales
with tabs[4]:
    st.header("Sink Sales (raw breakdown per sink)")
    if not selected_operator_address:
        st.info("Select a validator from the sidebar to see the sink sales time series.")
    else:
        sales_df = cached_sink_sales(selected_operator_address)
        if sales_df.empty:
            st.info("No validator_sink_sales data for this validator.")
        else:
            fig = px.line(
                sales_df,
                x="date",
                y="cumulative_sold_atom",
                color="sink_kind",
                line_group="sink_address",
                title=f"{selected_moniker} — cumulative sold per sink (ATOM)",
            )
            fig.update_traces(mode="lines+markers")
            st.plotly_chart(fig, use_container_width=True)
            cols = list(sales_df.columns)
            cols.remove("sink_name")
            cols.insert(cols.index("sink_address") + 1, "sink_name")
            st.dataframe(sales_df[cols], width="stretch")
