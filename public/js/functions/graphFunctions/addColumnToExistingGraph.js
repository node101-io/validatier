function addColumnToExistingGraph (params) {
  const { operatorAddress, data, timestamp, index, currency, decimals, usd_exchange_rate, symbol, graphDataMapping } = params;
  const graphWrapper = document.getElementById(`validator-graph-wrapper-${operatorAddress}`);

  const rootStyles = getComputedStyle(document.documentElement);
  const minValue = parseFloat(rootStyles.getPropertyValue('--min-value'));
  const maxValue = parseFloat(rootStyles.getPropertyValue('--max-value'));
  ...
  const allColumns = Array.from(document.querySelectorAll(`.column-wrapper-${operatorAddress}`));
  const columnIndex = parseInt(columnWrapper.getAttribute('index'));
  
  let inserted = false;
  
  for (let i = 0; i < allColumns.length; i++) {
    const eachColumnIndex = parseInt(allColumns[i].getAttribute('index'), 10);
  
    if (columnIndex <= eachColumnIndex) {
      graphWrapper.insertBefore(columnWrapper, allColumns[i]);
      inserted = true;
      break;
    }
  }
