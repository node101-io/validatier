function addColumnToExistingGraph (params) {
  ...
  const { operatorAddress, data, timestamp, index, currency, decimals, usd_exchange_rate, symbol, graphDataMapping, minValue, maxValue, graphWidth, dataFields, colors } = params;
  
  const graphWrapper = document.getElementById(`validator-graph-wrapper-${operatorAddress}`);
  addVerticalAxisLabels(graphWrapper, operatorAddress, minValue, maxValue, 10, currency, decimals, usd_exchange_rate, symbol);
  ...
  columnWrapper.style.width = `calc((${graphWidth}px - var(--vertical-axis-labels-width)) / var(--number-of-columns-${operatorAddress}))`;
  ...
  if (index % linePer == 0 && index % columnsPer != 0) columnWrapper.style.borderLeft = '1px dashed rgba(124, 112, 195, 1)';

  graphWrapper.appendChild(columnWrapper);

  document.documentElement.style.setProperty(
    `--graph-column-width-${operatorAddress.replace('\\@', '@')}`, 
    `calc((${graphWidth} - var(--vertical-axis-labels-width-int)) / var(--number-of-columns-${operatorAddress}))`
  );
