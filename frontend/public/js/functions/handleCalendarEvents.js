...
function updateDateInputs() {
  document.getElementById("header-range-bottom-block").innerHTML = new Date(
    selectedDateBottom
  ).toLocaleDateString("en-US");
  document.getElementById("periodic-query-bottom-timestamp").value =
    selectedDateBottom;
  document.getElementById("header-range-top-block").innerHTML = selectedDateTop
    ? new Date(selectedDateTop).toLocaleDateString("en-GB")
    : "pending";
...
function generateMonthContent(currentYearValue, currentMonthValue, startDay) {
  const currentChainFirstAvailableTime = document
  ...
  document.getElementById("current-month-and-year-display").innerHTML =
    new Date(currentYearValue, currentMonthValue - 1).toLocaleString("en-US", {
      month: "long",
      year: "numeric",
    });
  ...
  for (let i = 0; i < currentMonth.days.length; i++) {
    ...
    if (i == 0) {
      let weekCount = 0;
      const previousMonth = getMonthInfo(
        currentYearValue,
        currentMonthValue - 1
      );
      while (weekCount < eachDay.dayIndex) {
        const prevDateToBePushed =
          previousMonth.days[
            previousMonth.days.length - (eachDay.dayIndex - weekCount)
          ];
        ...
        if (
          ...
          new Date(currentChainFirstAvailableTime).getTime() >
            new Date(
              currentYearValue,
              currentMonthValue - 1,
              eachDay.date
        ...
        const dayToBeUsedAsDayInTheId =
          prevDateToBePushed.date.toString().length == 1
            ? "0" + prevDateToBePushed.date.toString()
            : prevDateToBePushed.date;
        const monthToBeUsedInTheId =
          (currentMonthValue - 1).toString().length == 1
            ? "0" + (currentMonthValue - 1).toString()
            : currentMonthValue - 1;

        date.setAttribute(
          "date",
          `${currentYearValue}-${monthToBeUsedInTheId}-${dayToBeUsedAsDayInTheId}`
        );
    ...
    const dayToBeUsedAsDayInTheId =
      eachDay.date.toString().length == 1
        ? "0" + eachDay.date.toString()
        : eachDay.date;
    const monthToBeUsedInTheId =
      currentMonthValue.toString().length == 1
        ? "0" + currentMonthValue.toString()
        : currentMonthValue;
    ...
    if (i == currentMonth.days.length - 1) {
      ...
      while (weekCount < 6 - eachDay.dayIndex) {
        ...
        const dayToBeUsedAsDayInTheId =
          nextDateToBePushed.date.toString().length == 1
            ? "0" + nextDateToBePushed.date.toString()
            : nextDateToBePushed.date;
        const monthToBeUsedInTheId =
          (currentMonthValue + 1).toString().length == 1
            ? "0" + (currentMonthValue + 1).toString()
            : currentMonthValue + 1;
...
function paintBlocksInBetween() {
  document
    .querySelectorAll(".middle-date")
    .forEach((each) => each.classList.remove("middle-date"));
  document
    .querySelectorAll(".selected-date-bottom")
    .forEach((each) => each.classList.remove("selected-date-bottom"));
  document
    .querySelectorAll(".selected-date-top")
    .forEach((each) => each.classList.remove("selected-date-top"));

  if (document.querySelector(`div[date='${selectedDateBottom}']`))
    document
      .querySelector(`div[date='${selectedDateBottom}']`)
      .classList.add("selected-date-bottom");
  if (document.querySelector(`div[date='${selectedDateTop}']`))
    document
      .querySelector(`div[date='${selectedDateTop}']`)
      .classList.add("selected-date-top");
  ...
  for (let i = 0; i < allDateElements.length; i++) {
    ...
    if (
      selectedDateBottom < eachDate.getAttribute("date") &&
      eachDate.getAttribute("date") < selectedDateTop
    )
      eachDate.classList.add("middle-date");
  }
}
...
function handleCalendarEvents(currentYearValue, currentMonthValue, startDay) {
  ...
  const clickEventListener = (event) => {
    if (
      event.target.classList.contains("selected-range") ||
      event.target.parentNode.classList.contains("selected-range") ||
      event.target.parentNode.parentNode.classList.contains("selected-range")
    ) {
      ...
      if (!target.classList.contains("selected-range-open")) {
        ...
        target.children[target.children.length - 1].style.transform =
          "rotateX(180deg) translateY(5px)";
      } else {
      ...
      event.target.classList.contains("left-wrapper-each-choice") &&
      ...
      document.getElementById("header-selected-range-description").innerHTML =
        event.target.innerHTML;

      selectedDateBottom = getDateRange(new Date().toISOString().split("T")[0])[
        event.target.id
      ].bottom;
      selectedDateTop = getDateRange(new Date().toISOString().split("T")[0])[
        event.target.id
      ].top;
      ...
      document
        .getElementById("selected-range")
        .classList.remove("selected-range-open");
      document
        .getElementById("selected-range")
        .parentNode.classList.remove("date-picker-open");
      document
        .getElementById("picker-main-wrapper")
        .classList.remove("picker-main-wrapper-open");
    }
  };
  ...
  document
    ...
    .addEventListener("click", previousMonthListener);
  eventListeners.push({
    element: document.getElementById("previous-month"),
    event: "click",
    listener: previousMonthListener,
  });

  document
    .getElementById("next-month")
    .addEventListener("click", nextMonthListener);
  eventListeners.push({
    element: document.getElementById("next-month"),
    event: "click",
    listener: nextMonthListener,
  });
