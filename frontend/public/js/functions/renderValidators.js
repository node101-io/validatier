...
function getValueWithDecimals(value, currency, exhange_rate, decimals) {
  ...
  return {
    nativeValue:
      currency != "%"
        ? `${shortNumberFormat(normalized)} ${currency}`
        : `${Math.round(value)}%`,
    usdValue:
      currency != "%"
        ? `$${shortNumberFormat(normalized * exchangeRate)}`
        : null,
  };
}
...
function generateValidatorRankingContent(response, sort_by, sortOrderMapping) {
  ...
  infoTableHeaderWrapper.classList.add(
    "each-table-header-wrapper",
    "each-table-header-validator-info-header"
  );
  ...
  const headers_array = JSON.parse(
    document
      .querySelector(".table-main-wrapper")
      .getAttribute("data-headers-array")
  );

  const exchangeRate = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_usd_exhange_rate");
  const currentChainSymbol = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_symbol");
  const currentChainDecimals = document
    .getElementById("network-switch-header")
  ...
  const createPercentageSoldTd = (value) => {
    ...
    td.classList.add(
      ...
      "validator-percentage-sold"
    );
    td.style.color = color;

    const span = document.createElement("span");
    span.innerHTML =
      value <= 100 ? `%${shortNumberFormat(Math.max(value, 0))}` : `%100.0`;
    td.appendChild(span);
  ...
  for (const header of headers_array) {
    ...
    if (header.popup_text) {
      ...
      infoHover.classList.add(
        "each-tooltip-info-hover",
        "each-tooltip-info-hover-table"
      );
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!validator.temporary_image_uri) img.style.borderRadius = "0";
    ...
    for (const header of headers_array) {
      ...
      if (header.id === "percentage_sold") {
        ...
        const value =
          header.id === "sold"
            ? Math.max(0, validator[header.id])
            : validator[header.id];
...
function renderValidators() {
  ...
  document.addEventListener("click", (event) => {
    const isHeaderClickedChecker =
      event.target.classList.contains("each-table-header-wrapper") ||
      event.target.parentNode.classList.contains("each-table-header-wrapper") ||
    ...
    while (
      isHeaderClickedChecker &&
      !target.classList.contains("each-table-header-wrapper")
    )
    ...
    const validatorsMainWrapper = document.getElementById(
      "validators-main-wrapper"
    );
    ...
    const topDate = document.getElementById(
    ...
    const chainIdentifier = document
      .getElementById("network-switch-header")
      .getAttribute("current_chain_identifier");

    const cacheResponse =
      rankingResponsesCache[
        bottomTimestamp + "." + topTimestamp + "." + chainIdentifier
      ];

    document
      .getElementById("validators-main-wrapper")
      .setAttribute("sort_by", sort_by);
    document
      .getElementById("validators-main-wrapper")
      .setAttribute("order", sortOrderMapping[sort_by]);
    ...
    if (cacheResponse) {
      ...
      return generateValidatorRankingContent(
        cacheResponse,
        sort_by,
        sortOrderMapping
      );
  ...
  document.body.addEventListener("click", (event) => {
    ...
    while (
      ...
      !target.classList.contains("validator-operator-address")
    )
      target = target.parentNode;
    if (!target.classList.contains("validator-operator-address")) return;

    navigator.clipboard.writeText(
      target.children[0].getAttribute("operator_address")
    );
    target.children[0].children[3].children[0].src = "/res/images/check.svg";
    setTimeout(() => {
      target.children[0].children[3].children[0].src =
        "/res/images/clipboard.svg";
    }, 1000);
  });
  ...
  document.addEventListener("mouseover", (event) => {
    ...
    document.querySelectorAll(".validator-moniker-text").forEach((each) => {
      if (
        each.innerHTML ==
        target.querySelector(".validator-moniker-text")?.innerHTML
      )
        return;
  ...
  document.addEventListener("click", (event) => {
    ...
    while (
      target != document.body &&
      !target.classList.contains("each-leaderboard-table-type-content")
    )
      target = target.parentNode;
    if (!target.classList.contains("each-leaderboard-table-type-content"))
      return;

    const type = target.getAttribute("type");
    const leaderboardTables = target.parentNode.parentNode.querySelectorAll(
    ...
    leaderboardTables.forEach((eachLeaderboardTable) => {
      if (eachLeaderboardTable.id.includes(leaderboardSortOrderMapping[type])) {
        ...
        ).children[0].className = "triangle-up-big";
        eachLeaderboardTable.classList.add("section-hidden");
      } else {
        target.querySelector(
          ".each-table-header-sort-indicators"
        ).children[0].className = "triangle-down-big";
