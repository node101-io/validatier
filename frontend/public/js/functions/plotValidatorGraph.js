function deltaValueInRangeSelection(
  eachDataField,
  column1,
  column2,
  direction,
  ...
  if (type != "average") {
    ...
    const deltaValue = !direction ? value_1 - value_2 : value_2 - value_1;

    return deltaValue;
  } else {
    const value_1 = parseFloat(column1.getAttribute(eachDataField));
    const value_1_prefix_sum = parseFloat(
      column1.getAttribute(`${eachDataField}_prefix_sum`)
    );
    const value_1_index = parseInt(column1.getAttribute("index"));

    const value_2 = parseFloat(column2.getAttribute(eachDataField));
    const value_2_prefix_sum = parseFloat(
      column2.getAttribute(`${eachDataField}_prefix_sum`)
    );
...
function plotValidatorGraph(params) {
  ...
  if (document.getElementById(`validator-graph-wrapper-${operatorAddress}`))
    document
      .getElementById(`validator-graph-wrapper-${operatorAddress}`)
      .remove();
  ...
  const graphWrapperHorizontalLabelsBackgroundAbsolute =
    document.createElement("div");
  if (type != "small") {
    graphWrapperHorizontalLabelsBackgroundAbsolute.classList.add(
      "graph-wrapper-horizontal-labels-background-absolute"
    );
    graphWrapper.appendChild(graphWrapperHorizontalLabelsBackgroundAbsolute);

    if (!validatorGraphEventListenersMapping[operatorAddress])
      validatorGraphEventListenersMapping[operatorAddress] = [];
    else
      validatorGraphEventListenersMapping[operatorAddress].forEach(
        (eachEventHandler) =>
          eachEventHandler.element.removeEventListener(
            eachEventHandler.event,
            eachEventHandler.handler
          )
      );
  ...
  const graphMouseDownHandler = (event) => {
    document
      .querySelectorAll(`.paint-bar-${operatorAddress}`)
      .forEach((each) => {
        each.style.width = "0";
        each.style.borderRight = `none`;
      });
    document
      .querySelectorAll(`.range-edge-${operatorAddress}`)
      .forEach((each) => each.classList.remove("range-edges-indicator"));
    ...
    while (
      ...
      !target.classList.contains("each-graph-column-wrapper")
    )
      target = target.parentNode;

    if (target.classList.contains("each-graph-column-wrapper"))
      return (validatorListenerVariablesMapping[
        operatorAddressM
      ].rangeInitialColumn = target);
    return (validatorListenerVariablesMapping[
      operatorAddressM
    ].rangeInitialColumn = document
      .getElementById(`validator-graph-wrapper-${operatorAddressM}`)
      .querySelectorAll(".each-graph-column-wrapper")[0]);
  };

  const graphMouseUpHandler = (event) => {
    const operatorAddressM = operatorAddress.replace("\\@", "@");

    validatorListenerVariablesMapping[operatorAddressM].isSelectingRange =
      false;
    if (
      !validatorListenerVariablesMapping[operatorAddressM].rangeInitialColumn ||
      !validatorListenerVariablesMapping[operatorAddressM].rangeFinalColumn ||
      ...
      document
        .querySelectorAll(`.paint-bar-${operatorAddress}`)
        .forEach((each) => (each.style.width = "0"));
      document
        .querySelectorAll(`.range-edge-${operatorAddress}`)
        .forEach((each) => each.classList.remove("range-edges-indicator"));
    } else {
      const deltaMapping = {};
      dataFields.forEach((eachDataField) => {
        const type = !["total_stake_sum", "price"].includes(eachDataField)
          ? "difference"
          : "average";
        const deltaValue = deltaValueInRangeSelection(
          eachDataField,
          validatorListenerVariablesMapping[operatorAddressM].rangeFinalColumn,
          validatorListenerVariablesMapping[operatorAddressM]
            .rangeInitialColumn,
          validatorListenerVariablesMapping[operatorAddressM]
            .isSelectionDirectionToLeft,
          type
        );
        ...
        metric.querySelector(
          ...
          eachDataField != "price"
            ? nativeValue
            : "$" + parseFloat(deltaMapping[eachDataField]).toFixed(2);
      ...
      const initialTimestamp = !validatorListenerVariablesMapping[
        operatorAddressM
      ].isSelectionDirectionToLeft
        ...
        : validatorListenerVariablesMapping[
            operatorAddressM
          ].rangeFinalColumn.getAttribute("timestamp");
      const timestamp = !validatorListenerVariablesMapping[operatorAddressM]
        .isSelectionDirectionToLeft
        ? validatorListenerVariablesMapping[
            operatorAddressM
          ].rangeFinalColumn.getAttribute("timestamp")
      ...
      if (
        ...
        validatorListenerVariablesMapping[
          operatorAddressM
        ].rangeFinalColumn.previousSibling
          .querySelectorAll(".graph-range-paint-bar")
          .forEach((eachPaintBar) => {
            eachPaintBar.style.width = "0px";
            eachPaintBar.style.borderRight = `none`;
