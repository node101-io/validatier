...
function handleValidatorSearch() {
  ...
  const allValidatorsMainWrapper = document.getElementById(
  ...
  validatorFilterInput.addEventListener("keyup", (event) => {
    const monikers = Array.from(
      document.querySelectorAll(".validator-moniker")
    ).map((each) => each.children[0].children[0].innerHTML);
    const operatorAddresses = Array.from(
      document.querySelectorAll(".operator-address-search")
    ).map((each) => each.id);

    document.documentElement.style.setProperty("--scrollbar-opacity", "1");
    if (!validatorFilterInput.value || validatorFilterInput.value.length <= 0)
      return document
        .querySelectorAll(".each-validator-wrapper")
        .forEach((each) => {
    ...
    for (let i = 0; i < monikers.length; i++) {
      ...
      if (
        ...
        eachOperatorAddress.includes(
          validatorFilterInput.value.trim().toLowerCase()
        )
      ) {
        document.getElementById(operatorAddresses[i]).style.display = "flex";
        document.getElementById(`${operatorAddresses[i]}-info`).style.display =
          "flex";
      }
    }
