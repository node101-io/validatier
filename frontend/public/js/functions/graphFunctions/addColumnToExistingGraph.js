...
function addColumnToExistingGraph(params) {
  ...
  const graphWrapper = document.getElementById(
    `validator-graph-wrapper-${operatorAddress}`
  );
  if (type != "small") {
    if (dataFields[0] != "percentage_sold")
      addVerticalAxisLabels(
        graphWrapper,
        operatorAddress,
        minValue,
        maxValue,
  ...
  if (type != "small") {
    ...
    eachDataIndicatorVerticalLine.classList.add(
  ...
  dataFields.forEach((eachDataField, i) => {
    ...
    point.classList.add(
      `${eachDataField}-graph-data-element-${operatorAddress}`
    );
    ...
    if (type != "small") {
      ...
      paintBar.classList.add(
        "graph-range-paint-bar",
        `paint-bar-${operatorAddress.replace("\\@", "@")}`,
        `${eachDataField}-graph-data-paint-bar-${operatorAddress}`
      );
  ...
  if (type != "small") {
    ...
    if (index % columnsPer == 0) {
      horizontalLabelDataMapping[
        operatorAddress
      ].columnToAppendHorizontalLabel = columnWrapper;
    }
    if (index % linePer == 0 && index % columnsPer != 0) {
      ...
      if (!horizontalLabelDataMapping[operatorAddress].currentIntervalLeft) {
        horizontalLabelDataMapping[operatorAddress].currentIntervalLeft =
          timestamp;
      } else if (
        !horizontalLabelDataMapping[operatorAddress].currentIntervalRight
      ) {
        horizontalLabelDataMapping[operatorAddress].currentIntervalRight =
          timestamp;
        generateSingleHorizontalAxisLabel(
          horizontalLabelDataMapping[operatorAddress]
        );
  ...
  dataFields.forEach((eachDataField) => {
    const value = data[eachDataField];
    if (
      ["price", "total_stake_sum"].includes(eachDataField) &&
      columnWrapper.previousSibling
    ) {
      if (
        !columnWrapper.previousSibling.classList.contains(
          "each-graph-column-wrapper"
        )
      ) {
        columnWrapper.setAttribute(`${eachDataField}_prefix_sum`, value);
      } else {
        const previousColumnPrefixSum =
          columnWrapper.previousSibling.getAttribute(
            `${eachDataField}_prefix_sum`
          );
        columnWrapper.setAttribute(
          `${eachDataField}_prefix_sum`,
          parseFloat(previousColumnPrefixSum) + value
        );
      }
    }
  });
