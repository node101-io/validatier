function addColumnEventListener(
  operatorAddress,
  dataFields,
  colors,
  currency,
  ...
  const resetMetrics = () => {
    Object.keys(averageMapping).forEach((eachDataField) => {
      ...
      const { nativeValue, usdValue } = getValueWithDecimals(
        value,
        eachDataField != "percentage_sold" ? currency : "%",
        exchange_rate,
        decimals
      );

      const currentTitleContent = metric.querySelector(
        ".each-metric-content-wrapper-header-title"
      ).innerHTML;

      if (eachDataField == "total_stake_sum")
        metric.querySelector(
          ".each-metric-content-wrapper-header-title"
        ).innerHTML = currentTitleContent.replace("Total", "Average");
      else if (
        eachDataField == "price" &&
        !currentTitleContent.includes("Average ")
      )
        metric.querySelector(
          ".each-metric-content-wrapper-header-title"
        ).innerHTML = "Average " + currentTitleContent;
  ...
  const columnMouseHandler = (event) => {
    ...
    document.querySelectorAll(`.${visibleClassName}`).forEach((each) => {
      if (
        !each.classList.contains("range-value-display") &&
        !each.classList.contains("range-edges-indicator")
      ) {
    ...
    while (
      ...
      !target.classList.contains(`column-wrapper-${operatorAddress}`)
    )
    ...
    columnWrapper
      .querySelectorAll(".each-data-point")
      .forEach((eachDataPoint) => {
        eachDataPoint.classList.add(
          "each-data-point-hovered",
          visibleClassName.replace("\\@", "@")
        );
      });

    columnWrapper
      .querySelector(".each-data-delta-vertical-line")
      .classList.add(
        "each-data-delta-vertical-line-visible",
        visibleClassName.replace("\\@", "@")
      );
    columnWrapper
      .querySelector(".each-data-indicator-vertical-line")
      .classList.add(
        "each-data-indicator-vertical-line-visible",
        visibleClassName.replace("\\@", "@")
      );

    const showPercentageChange = document.body.getAttribute(
    ...
    if (
      ...
      dataFields.forEach((eachDataField) => {
        const { nativeValue, usdValue } = getValueWithDecimals(
          columnWrapper.getAttribute(eachDataField),
          eachDataField != "percentage_sold" ? currency : "%",
          exchange_rate,
          decimals
        );
        ...
        if (eachDataField == "total_stake_sum")
          metric.querySelector(
            ".each-metric-content-wrapper-header-title"
          ).innerHTML = currentTitleContent.replace("Average", "Total");
        else if (eachDataField == "price")
          metric.querySelector(
            ".each-metric-content-wrapper-header-title"
          ).innerHTML = currentTitleContent.replace("Average", "");
        ...
        metric.querySelector(
          ...
          eachDataField != "price"
            ? nativeValue
            : "$" +
              parseFloat(columnWrapper.getAttribute(eachDataField)).toFixed(2);
        if (eachDataField != "price")
          metric.querySelector(
            ".each-metric-content-wrapper-content-value-usd"
          ).innerHTML = usdValue;
    ...
    if (!validatorListenerVariablesMapping[operatorAddressM].isSelectingRange)
      return;
    ...
    if (
      ...
      while (current != target) {
        ...
        if (
          target.getAttribute("timestamp") <
          validatorListenerVariablesMapping[
            operatorAddressM
          ].rangeInitialColumn.getAttribute("timestamp")
        ) {
          validatorListenerVariablesMapping[
            operatorAddressM
          ...
          validatorListenerVariablesMapping[operatorAddressM].rangeInitialColumn
            .querySelectorAll(".graph-range-paint-bar")
            .forEach((eachPaintBar) => {
          ...
          dataFields.forEach((eachDataField) => {
            const groupId = subplotGroupMapping
              ? subplotGroupMapping[eachDataField]
              : 0;
            const addOn = subplotGroupMapping
              ? `-${eachDataField != "price" ? groupId : "0"}`
              : "";
            const numberOfGroups = subplotGroupMapping
              ? subplotGroupMapping["number_of_groups"]
              : 1;
            bottomMapping[eachDataField] =
              `calc(${(100 / numberOfGroups) * groupId}% + ((${current.nextSibling.getAttribute(eachDataField)} - var(--min-value-${operatorAddress}${addOn})) / (var(--max-value-${operatorAddress}${addOn}) - var(--min-value-${operatorAddress}${addOn}))) * ${100 / numberOfGroups}%)`;
          });
        } else {
          validatorListenerVariablesMapping[
            operatorAddressM
          ...
          dataFields.forEach((eachDataField) => {
            ...
            const numberOfGroups = subplotGroupMapping
              ...
              : 1;
            bottomMapping[eachDataField] =
              `calc(${(100 / numberOfGroups) * groupId}% + ((${current.getAttribute(eachDataField)} - var(--min-value-${operatorAddress}${addOn})) / (var(--max-value-${operatorAddress}${addOn}) - var(--min-value-${operatorAddress}${addOn}))) * ${100 / numberOfGroups}%)`;
          });
        }
        ...
        const dataMapping = {};
        dataFields.forEach((eachDataField) => {
          dataMapping[eachDataField] = parseFloat(
            current.getAttribute(eachDataField)
          );
        });
        ...
        current
          .querySelectorAll(".graph-range-paint-bar")
          .forEach((eachPaintBar, i) => {
            const dataField = dataFields[i];
            const color = colors[i];

            eachPaintBar.style.width =
              angleHypotenuseMapping[dataField].hypotenuse;
    ...
    const angleHypotenuseMappingAnglesOnly = getAngleBetweenTwoPoints(
      current,
      current.nextSibling,
      operatorAddress,
      dataFields,
    ...
    if (
      ...
      const angleHypotenuseMappingHypotenuseOnly = getAngleBetweenTwoPoints(
        current,
        current.nextSibling,
        operatorAddress,
        dataFields,
      ...
      dataFields.forEach((eachDataField) => {
        ...
        const numberOfGroups = subplotGroupMapping
          ...
          : 1;
        bottomMapping[eachDataField] =
          `calc(${(100 / numberOfGroups) * groupId}% + ((${columnWrapper.nextSibling.getAttribute(eachDataField)} - var(--min-value-${operatorAddress}${addOn})) / (var(--max-value-${operatorAddress}${addOn}) - var(--min-value-${operatorAddress}${addOn}))) * ${100 / numberOfGroups}%)`;
      });
      ...
      columnWrapper
        ...
        .forEach((eachPaintBar, i) => {
          const hypotenuse =
            angleHypotenuseMappingHypotenuseOnly[dataFields[i]].hypotenuse;
      ...
      dataFields.forEach((eachDataField) => {
        ...
        const numberOfGroups = subplotGroupMapping
          ...
          : 1;
        bottomMapping[eachDataField] =
          `calc(${(100 / numberOfGroups) * groupId}% + ((${columnWrapper.getAttribute(eachDataField)} - var(--min-value-${operatorAddress}${addOn})) / (var(--max-value-${operatorAddress}${addOn}) - var(--min-value-${operatorAddress}${addOn}))) * ${100 / numberOfGroups}%)`;
      });
  ...
  validatorGraphEventListenersMapping[operatorAddress].push({
    event: "mousemove",
    handler: columnMouseHandler,
