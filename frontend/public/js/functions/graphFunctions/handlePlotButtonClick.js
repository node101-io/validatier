...
function changeSummaryGraph(target) {
  document
    .querySelectorAll(".each-sub-menu-link-content")
    .forEach((eachLink) => {
  ...
  history.pushState(
    { page: "dashboard" },
    "dashboard",
    `/${target.id != "dashboard" ? target.id : ""}`
  );

  document
    .getElementById("all-validators-main-wrapper")
    .classList.remove("section-hidden");
  document
    .getElementById("validators-leaderboards-all-wrapper")
    .classList.remove("section-hidden");
  document
    .getElementById("network-summary-main-wrapper")
    .classList.remove("section-hidden");
  document
    .getElementById("intro-main-wrapper")
    .classList.remove("section-hidden");
  document
    .getElementById("validator-details-main-wrapper")
  ...
  const lastVisitedOperatorAddress = sessionStorage.getItem(
    "last_visited_operator_address"
  );
  ...
  const validatorFilterInput = document.getElementById(
  ...
  document.getElementById("all-main-wrapper").scrollTo({
    top:
      document.getElementById(lastVisitedOperatorAddress).offsetTop +
      document.getElementById("all-validators-main-wrapper").offsetTop -
...
const validator_stats = [
  ...
  {
    id: "validator-stat-percentage-sold",
  ...
  {
    id: "validator-stat-commission-rate",
    title: "Commission",
    field: "commission_rate",
    usdContent: false,
    additional_class: "summary-percentage-text-native",
    helperText: "fee from rewards",
    helperType: "text",
  },
];
...
function generateGraph(validator) {
  ...
  selectedRange.style.border =
    "var(--selected-range-border-size) solid var(--selected-range-border-color)";
  ...
  headerBannerWrapper.style.marginLeft =
    "var(--header-banner-wrapper-margin-left)";
  ...
  const priceGraphData = JSON.parse(
    document.body.getAttribute("priceGraphData")
  );

  document.documentElement.style.setProperty(
    `--number-of-columns-${operatorAddress}`,
    89
  );

  const decimals = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_decimals");
  const usd_exchange_rate = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_usd_exhange_rate");
  const symbol = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_symbol");
  const chainId = document
    .getElementById("network-switch-header")
    .getAttribute("current_chain_id");
  const currency = "native";

  document.getElementById("validator-details-website").href =
    `${validator.website || ""}`;
  document.getElementById("validator-details-explorer").href =
    `https://www.mintscan.io/cosmos/validators/${validator.operator_address || ""}`;
  document.getElementById("validator-details-stake").href =
    `https://wallet.keplr.app/chains/${chainIdentifier == "cosmoshub" ? "cosmos-hub" : chainIdentifier}?modal=validator&chain=${chainId}&validator_address=${validator.operator_address || ""}`;

  validator_stats.forEach((stat) => {
    if (stat.field == "operator_address") {
      const validatorOperatorAddressContent = document.getElementById(
        "validator-details-operator-address"
      );
      validatorOperatorAddressContent.setAttribute(
        "operator_address",
        validator[stat.field]
      );

      validatorOperatorAddressContent.children[0].innerHTML =
        validator.operator_address.slice(0, 4);
      validatorOperatorAddressContent.children[1].children[0].innerHTML =
        validator.operator_address.slice(
          4,
          (validator.operator_address.length - 4) / 2
        );
      validatorOperatorAddressContent.children[1].children[2].innerHTML =
        validator.operator_address.slice(
          (validator.operator_address.length - 4) / 2,
          validator.operator_address.length - 4
        );
      validatorOperatorAddressContent.children[2].innerHTML =
        validator.operator_address.slice(
          validator.operator_address.length - 4,
          validator.operator_address.length
        );
      return;
    ...
    if (stat.field == "commission_rate") {
      document.getElementById(`${stat.id}-native`).innerHTML = formatCommission(
        validator[stat.field]
      );
      ...
      wrapper.children[1].innerHTML =
        "%" + shortNumberFormat(validator["self_stake_ratio"]);
    } else {
      document.getElementById(`${stat.id}-native`).innerHTML =
        "%" +
        Math.round(
          shortNumberFormat(
            Math.min(Math.max(0.0, validator[stat.field]), 100)
          ) * 100
        ) /
          100;
    }
    ...
    if (stat.helperType == "text")
      ...
      document.getElementById(`${stat.id}-helper`).innerHTML =
        "→" +
        Math.round(
          (validator[stat.field] / summaryData[`initial_${stat.field}`]) * 100
        ) +
        "%";
      ...
      if (stat.field == "self_stake") {
        const ssRank =
          [...validators]
            .sort((a, b) => b["self_stake"] - a["self_stake"])
            .findIndex(
              (v) => v.operator_address === validator.operator_address
            ) + 1;
        const ssrRank =
          [...validators]
            .sort((a, b) => b["self_stake_ratio"] - a["self_stake_ratio"])
            .findIndex(
              (v) => v.operator_address === validator.operator_address
            ) + 1;
        ...
        const suffix =
          suffixArray[
            parseInt(averageRank.toString()[averageRank.toString().length - 1])
          ];

        document.getElementById(`${stat.id}-helper`).innerHTML =
          averageRank + `${suffix} out of ` + validators.length;
      } else {
        const rank =
          [...validators]
            .sort((a, b) => a[stat.field] - b[stat.field])
            .findIndex(
              (v) => v.operator_address === validator.operator_address
            ) + 1;
        const suffix =
          suffixArray[parseInt(rank.toString()[rank.toString().length - 1])];
        document.getElementById(`${stat.id}-helper`).innerHTML =
          rank + `${suffix} out of ` + validators.length;
      }
    }
  });

  const bottomDate = document.getElementById(
  ...
  const graphWrapper = plotValidatorGraph({
    type: "validator",
    operatorAddress: operatorAddress.replace("@", "\\@"),
    decimals,
    usd_exchange_rate,
    symbol,
    validatorGraphEventListenersMapping,
    dataFields,
    graphContainer,
    summaryData,
  });
  const graphWidth = window
    .getComputedStyle(graphWrapper, null)
    .getPropertyValue("width")
    .replace("px", "");

  const queryString = new URLSearchParams(requestData).toString();
  const eventSource = new EventSource(
    `/validator/get_graph_data?${queryString}`
  );
  ...
  const subplotGroupArray = [["price"], ["total_sold"], ["total_stake_sum"]];
  ...
  eventSource.onmessage = (event) => {
    ...
    if (response.message == "finished") {
      ...
      document
        .querySelectorAll(`.column-wrapper-${operatorAddress}`)
        .forEach((eachColumn) => {
          ...
          for (let i = 0; i < inactivityIntervals.length; i += 2) {
            if (
              ...
              eachColumn.classList.add(
                "each-graph-column-wrapper-inactivity-indicator"
              );
            }
          }
        });
    ...
    dataFields.forEach((eachDataField) => {
      const { nativeValue, usdValue } = getValueWithDecimals(
        data[eachDataField],
        eachDataField != "percentage_sold" ? symbol : "%",
        usd_exchange_rate,
        decimals
      );
      const metric = document.getElementById(
        `validator-metric-${eachDataField}`
      );

      metric.querySelector(
        ".each-metric-content-wrapper-content-value-native"
      ).innerHTML =
        eachDataField != "price" ? nativeValue : "$" + data[eachDataField];
      if (eachDataField != "price")
        metric.querySelector(
    ...
    for (let i = 0; i < subplotGroupArray.length; i++) {
      ...
      let { minValue: min, maxValue: max } = calculateMaxAndMinValue(
        graphDataMapping,
        eachSubplotGroup
      );

      // Değişmesi gerekirse buradan değiştir
      if (!["total_stake_sum", "price"].includes(eachSubplotGroup[0]))
        minValue = min;
      else minValue = 0;
      maxValue = max;
      ...
      document.documentElement.style.setProperty(
        `--min-value-${operatorAddress}-${i}`,
        minValue
      );
      document.documentElement.style.setProperty(
        `--max-value-${operatorAddress}-${i}`,
        maxValue
      );
    }
    ...
    if (
      ...
      insertedColumn.previousSibling.classList.contains(
        "each-graph-column-wrapper"
      )
    ) {
      adjustLineWidthAndAngle(
        insertedColumn.previousSibling,
        insertedColumn,
        operatorAddress.replace("@", "\\@"),
        dataFields,
      ...
      document.documentElement.style.setProperty(
        `--column-height-${operatorAddress}`,
        insertedColumn.offsetHeight
      );
    }
  };
...
function handlePlotButtonClick() {
  document.body.addEventListener("click", async (event) => {
    ...
    while (
      ...
      !target.classList.contains("each-leaderboard-table-validator-wrapper")
    )
      target = target.parentNode;
    if (!target.classList.contains("each-leaderboard-table-validator-wrapper"))
      return;
  ...
  document.body.addEventListener("click", async (event) => {
    ...
    while (
      target != document.body &&
      !target.classList.contains("each-validator-wrapper")
    )
      target = target.parentNode;
  ...
  window.onresize = () => {
    ...
    isResizing = true;
    const graphWrappersArray = document.querySelectorAll(
    ...
    setTimeout(() => {
      for (let i = 0; i < graphWrappersArray.length; i++) {
        ...
        const operatorAddress =
          eachGraphWrapper.getAttribute("operator_address");
