function handleSummaryGraphActions() {
  ...
  const openContentClassName =
    "each-network-summary-network-graph-content-dropdown-content-open";
  const dropdownArrowClassName =
    "each-network-summary-network-graph-content-dropdown-arrow";

  document.body.addEventListener("click", (event) => {
    let target = event.target;
    while (
      target != document.body &&
      !target.classList.contains("each-network-summary-select-option")
    )
      target = target.parentNode;
    if (!target.classList.contains("each-network-summary-select-option"))
      return;

    document
      .querySelector(".each-network-summary-select-option-selected")
      .classList.remove("each-network-summary-select-option-selected");
    target.classList.add("each-network-summary-select-option-selected");

    const networkSummaryGraphContainer = document.getElementById(
      "network-summary-graph-container"
    );
    const currentDataFields = JSON.parse(
      networkSummaryGraphContainer.getAttribute("currentDataFields")
    );
    const currentColors = JSON.parse(
      networkSummaryGraphContainer.getAttribute("currentColors")
    );
  ...
  document.body.addEventListener("click", (event) => {
    ...
    while (
      target != document.body &&
      !target.classList.contains(headerClassName) &&
      !target.classList.contains(contentClassName)
    )
      target = target.parentNode;
    ...
    if (
      ...
      target.parentNode
        .querySelectorAll("." + dropdownArrowClassName)
        .forEach((each) => {
          each.style.transform = "rotateX(0deg)";
          each.style.marginTop = "2px";
        });
      target.parentNode
        .querySelectorAll("." + contentClassName)
        .forEach((each) => each.classList.remove(openContentClassName));
      return;
    }
    ...
    const dropdownArrow = networkSwitchDropdown.parentNode.querySelector(
      ".each-network-summary-network-graph-content-dropdown-arrow"
    );
    ...
    if (!networkSwitchDropdown.classList.contains(openContentClassName)) {
      ...
      target.parentNode
        .querySelectorAll("." + contentClassName)
        .forEach((each) => each.classList.add(openContentClassName));
    } else {
...
function createNetworkSummaryGraph(dataFields, colors) {
  ...
  const targetCacheSummaryGraphData = JSON.parse(
    document.body.getAttribute("summaryGraphData")
  );
  ...
  while (priceGraphData.length != targetCacheSummaryGraphData.length) {
    if (priceGraphData.length > targetCacheSummaryGraphData.length)
      priceGraphData.pop();
    else targetCacheSummaryGraphData.pop();
  }

  document
    .querySelectorAll(
      ".each-network-summary-network-graph-content-each-dropdown"
    )
    .forEach((each) => {
      if (!each.classList.contains("leaderboard-dropdown-option"))
        each.classList.add("section-hidden");
    });
  document
    .querySelectorAll(".each-metric-content-wrapper")
    .forEach((each) => each.classList.add("section-hidden"));
  dataFields.forEach((eachDataField) => {
    document
      .getElementById(`summary-graph-dropdown-option-${eachDataField}`)
      .classList.remove("section-hidden");
    document
      .getElementById(`summary-metric-${eachDataField}`)
      .classList.remove("section-hidden");
  });
  ...
  document.documentElement.style.setProperty(
    ...
    targetCacheSummaryGraphData.length - 1
  );
  ...
  const subplotGroupArray =
    dataFields[0] != "percentage_sold"
      ? [["price"], ["total_sold"], ["total_stake_sum"]]
      : [["percentage_sold"], ["price"]];
    ...
    dataFields.forEach((eachDataField) => {
      if (!currentSumMapping[eachDataField])
        !["total_stake_sum"].includes(eachDataField)
          ...
          : (currentSumMapping[eachDataField] =
              summaryData[`initial_${eachDataField}`]);

      if (eachDataField != "price" && eachDataField != "percentage_sold")
  ...
  const rightVerticalAxisWrapper = document.getElementById(
    "summary-graph-vertical-axis-labels1"
  );
  if (rightVerticalAxisWrapper)
    graphWrapper.appendChild(rightVerticalAxisWrapper);

  const graphColumns = graphWrapper.querySelectorAll(
    ".each-graph-column-wrapper"
  );
...
function createSmallGraphs() {
  const smallGraphData = JSON.parse(
    document.body.getAttribute("smallGraphData")
  );
  const graphsDataFields = [["self_stake_sum"], ["average_self_stake_ratio"]];
  ...
  graphsDataFields.forEach((dataFields) => {
    ...
    document.documentElement.style.setProperty(
      ...
      smallGraphData.length - 1
    );

    const graphDataMapping = {};
    const graphContainer = document.getElementById(
      `small-graph-${operatorAddress}`
    );
    const graphWrapper = plotValidatorGraph({
      type: "small",
      operatorAddress,
      ...
      symbol: null,
      validatorGraphEventListenersMapping: null,
      dataFields,
      graphContainer,
    });
    const graphWidth = window
    ...
    for (let i = 0; i < smallGraphData.length; i++) {
      ...
      graphDataMapping[i] = data;
      let { minValue, maxValue } = calculateMaxAndMinValue(
        graphDataMapping,
        dataFields
      );
      if (dataFields[0] == "average_self_stake_ratio") maxValue = 100;

      document.documentElement.style.setProperty(
        `--min-value-${operatorAddress}`,
        minValue
      );
      document.documentElement.style.setProperty(
        `--max-value-${operatorAddress}`,
        maxValue
      );
