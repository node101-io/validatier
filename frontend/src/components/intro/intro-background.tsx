"use client";

export default function IntroBackgroundSVG() {
  return (
    <svg
      id="intro-svg"
      width="1440"
      height="550"
      viewBox="0 0 1440 550"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      style={{
        width: "100%",
        height: "105%",
        display: "block"
      }}
    >
      <path
        d="M1114.55 531.875c217.9 0 394.55-118.903 394.55-265.577S1332.45.721 1114.55.721c-217.912 0-394.561 118.903-394.561 265.577s176.649 265.577 394.561 265.577Z"
        stroke="url(#a)"
        strokeMiterlimit="10"
      />
      <path
        d="M984.874 532.621c217.906 0 394.556-118.903 394.556-265.577S1202.78 1.467 984.874 1.467c-217.908 0-394.556 118.903-394.556 265.577s176.648 265.577 394.556 265.577Z"
        stroke="url(#b)"
        strokeMiterlimit="10"
      />
      <path
        d="M807.545 532.621c217.905 0 394.555-118.903 394.555-265.577S1025.45 1.467 807.545 1.467c-217.907 0-394.556 118.903-394.556 265.577s176.649 265.577 394.556 265.577Z"
        stroke="url(#c)"
        strokeMiterlimit="10"
      />
      <path
        d="M630.216 532.621c217.908 0 394.554-118.903 394.554-265.577S848.124 1.467 630.216 1.467 235.66 120.37 235.66 267.044s176.649 265.577 394.556 265.577Z"
        stroke="url(#d)"
        strokeMiterlimit="10"
      />
      <path
        d="M452.888 532.621c217.907 0 394.556-118.903 394.556-265.577S670.795 1.467 452.888 1.467C234.98 1.467 58.331 120.37 58.331 267.044S234.98 532.621 452.888 532.621Z"
        stroke="url(#e)"
        strokeMiterlimit="10"
      />
      <path
        d="M324.324 532.621c217.908 0 394.557-118.903 394.557-265.577S542.232 1.467 324.324 1.467c-217.907 0-394.556 118.903-394.556 265.577s176.649 265.577 394.556 265.577Z"
        stroke="url(#f)"
        strokeMiterlimit="10"
      />
      <path
        d="m-70.21 267.044 1580.44-1.478"
        stroke="url(#g)"
        strokeMiterlimit="10"
      />
      <defs>
        <linearGradient
          id="a"
          x1="1114.55"
          y1=".721"
          x2="1114.55"
          y2="531.875"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="b"
          x1="984.874"
          y1="1.467"
          x2="984.874"
          y2="532.621"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="c"
          x1="807.545"
          y1="1.467"
          x2="807.545"
          y2="532.621"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="d"
          x1="630.216"
          y1="1.467"
          x2="630.216"
          y2="532.621"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="e"
          x1="452.888"
          y1="1.467"
          x2="452.888"
          y2="532.621"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="f"
          x1="324.324"
          y1="1.467"
          x2="324.324"
          y2="532.621"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".327"
            stopColor="#E4E4E4"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".563"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop
            className="intro-stop-fade"
            offset=".774"
            stopColor="#E7E7E7"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
        <linearGradient
          id="g"
          x1="1521.9"
          y1="267.036"
          x2="-70.21"
          y2="265.566"
          gradientUnits="userSpaceOnUse"
        >
          <stop className="intro-stop-fixed" stopColor="#fff" />
          <stop
            className="intro-stop-fade"
            offset=".489"
            stopColor="#D0D0D0"
            stopOpacity="0"
          />
          <stop className="intro-stop-fixed" offset="1" stopColor="#fff" />
        </linearGradient>
      </defs>
    </svg>
  );
}
