...
export default async function Home() {
    ...
    const cacheResult = await new Promise<CacheInterface | Omit<CacheInterface, "export">>((resolve) => {
    ...
    const price = chains.find(chain => chain.name === "cosmoshub")?.usd_exchange_rate ?? 0;


	const { weightedSum, length } = cacheResult.summary_graph.reduce((acc, each, idx, arr) => {
    ...
    const averageDelegation = cacheResult.summary_data.initial_total_stake_sum + weightedSum / length;
    const totalSoldAmount = cacheResult.summary_graph.reduce((acc, each) => acc + each.total_sold, 0);
    const averagePrice = cacheResult.price_graph.reduce((acc, each) => acc + each, 0) / cacheResult.price_graph.length;

    const soldData: number[] = [];
    for (let i = 0; i < cacheResult.summary_graph.length; i++)
        soldData.push(cacheResult.summary_graph[i].total_sold / 1_000_000 + (soldData[i - 1] ?? 0));

    const delegationData: number[] = [cacheResult.summary_data.initial_total_stake_sum / 1_000_000];
    for (let i = 0; i < cacheResult.summary_graph.length; i++)
        delegationData.push((cacheResult.summary_graph[i].total_stake_sum) / 1_000_000 + delegationData[i]);
