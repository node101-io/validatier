  ...
  function listenInterval (bottom_block_height: number, top_block_height: number) {
    if (bottom_block_height > max_block_height) {
      ...
      return setTimeout(() => {
        Chain.findChainByIdentifier({ chain_identifier: chain_identifier }, (err, chain) => {
          ...
          processBlocks(end, chain.last_available_block_height, chain_identifier);
        })
    ...
    listenForEvents(bottom_block_height, top, chain_identifier, (err, result) => {
      ...
      if (result.inserted_validator_addresses && result.inserted_validator_addresses.length) 
        logMessages.push(`CREATED | Validators with addresses: ${result.inserted_validator_addresses}`);
      if (result.updated_validator_addresses && result.updated_validator_addresses.length) 
        logMessages.push(`UPDATED | Validators with addresses: ${result.updated_validator_addresses}`);
      if (result.saved_composite_event_block_heights && result.saved_composite_event_block_heights.length)
        logMessages.push(`SAVED | Composite Event Blocks in these block heights: ${result.saved_composite_event_block_heights}`);
      if (result.saved_active_validators && result.saved_active_validators.active_validators.length)
        logMessages.push(`SAVED | Active Validator set for ${result.saved_active_validators.month}/${result.saved_active_validators.year}: ${result.saved_active_validators.active_validators.length} currently in active list`);
