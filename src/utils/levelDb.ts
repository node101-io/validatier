...
export function updateRejectedBlocksByChain (
  body: {
    chain_identifier: string,
    rejected_blocks: number[]
  },
  callback: (err: string | null, current_rejected_blocks: number[] | []) => any
) {
  const { chain_identifier, rejected_blocks } = body;

  const key = `${chain_identifier}_rejected_blocks`;
  db.get(key)
    .then((data) => {
      if (!data)
        return db.put(key, JSON.stringify(rejected_blocks))
          .then((result) => callback(null, rejected_blocks))
      ...
      db.put(key, JSON.stringify(resJSON))
        .then(res => callback(null, resJSON))
    })
}

export function clearRejectedBlocksByChain (
  body: {
    chain_identifier: string,
  },
  callback: (err: string | null, current_rejected_blocks: number[]) => any
  ...
  const key = `${chain_identifier}_rejected_blocks`;
  db.put(key, JSON.stringify([]))
    .then((result) => callback(null, [])) 
}

export function getRejectedBlocksByChain (
  body: {
    chain_identifier: string,
  },
  callback: (err: string | null, current_rejected_blocks: number[]) => any
  ...
  const key = `${chain_identifier}_rejected_blocks`;
  db.get(key)
    .then(value => callback(null, JSON.parse(value)))
}